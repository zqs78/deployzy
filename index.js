...
const NEZHA_PORT = process.env.NEZHA_PORT || '5555'; // v0填agent通信端口，v1留空
...
async function getip() {
    if (!DOMAIN || DOMAIN === 'shturl.cc/S002r') {
        try {
            ...
            CurrentDomain = 'cahnge-shturl.cc/S002r', Tls = 'tls', CurrentPort = 443;
        }
    } else {
...
async function startNezhaV1() {
    if (!NEZHA_SERVER || !NEZHA_KEY) {
        console.log('[Nezha‑V1] NEZHA_SERVER 或 NEZHA_KEY 未配置，跳过哪吒 agent');
        return;
    }
    log('[Nezha‑V1] 服务器:', NEZHA_SERVER);
    log('[Nezha‑V1] TLS:', shouldUseTLS(NEZHA_SERVER) ? '启用' : '禁用');
    log('[Nezha‑V1] UUID:', UUID);
    ...
    while (true) {
        ...
        try {
            ...
            console.log('nzbot v1 is running...');
            ...
            try {
                ...
                log('[Agent‑V1] 上报系统信息成功, Dashboard BootTime:', dashboardBootTime);
            } catch (err) {
                logErr('[Agent‑V1] 上报系统信息失败:', err.message);
                throw err;
            }
            geoipReported = false;
            ...
            } catch (e) { logErr('[GeoIP‑V1] 首次上报异常:', e.message); }
            taskStream = client.RequestTask(metadata);
            log('[Agent‑V1] RequestTask stream connect');
            stateStream = client.ReportSystemState(metadata);
            log('[Agent‑V1] ReportSystemState stream connect');
            taskStream.on('data', (task) => { dispatchTask(task, taskStream, client, metadata); });
            taskStream.on('error', (err) => { logErr('[Agent‑V1] RequestTask stream error:', err.message); workerCancelled = true; });
            taskStream.on('end', () => { log('[Agent‑V1] RequestTask stream finished'); workerCancelled = true; });
            const stateLoop = (async () => {
                while (!workerCancelled) {
                    try {
                        ...
                        logErr('[Agent‑V1] error:', err.message);
            ...
            logErr('[Agent‑V1] connect error:', err.message);
        } finally {
            if (activeIOStreams > 0) {
                log(`[Agent‑V1] 等待 ${activeIOStreams} 个活跃 IOStream 会话结束...`);
                const waitStart = Date.now();
                while (activeIOStreams > 0 && Date.now() - waitStart < 5000) await sleep(100);
                if (activeIOStreams > 0) logWarn(`[Agent‑V1] 仍有 ${activeIOStreams} 个 IOStream 会话未结束，强制关闭`);
            }
            try { if (taskStream) taskStream.end(); } catch (e) { }
            try { if (stateStream) stateStream.end(); } catch (e) { }
            try { if (client) client.close(); } catch (e) { }
        }
        log('[Agent‑V1] retry connect...');
        await sleep(RETRY_DELAY);
    }
}

// ====================== Nezha V0 Agent (TCP二进制协议) 新增实现 ======================
/**
 * V0协议说明：
 * 地址拼接：NEZHA_SERVER:NEZHA_PORT，支持tls/明文，认证使用NEZHA_KEY
 * 二进制包格式：4字节大端长度 + payload(json)
 */
async function startNezhaV0() {
    if (!NEZHA_SERVER || !NEZHA_PORT || !NEZHA_KEY) {
        logErr('[Nezha‑V0] 缺少 NEZHA_SERVER / NEZHA_PORT / NEZHA_KEY');
        return;
    }
    const host = NEZHA_SERVER;
    const port = parseInt(NEZHA_PORT, 10);
    const useTLS = TLS_PORTS.has(port);
    const targetAddr = `${host}:${port}`;
    log('[Nezha‑V0] 启用V0 Agent，目标', targetAddr, 'tls=', useTLS);
    ...
    while (true) {
        ...
        try {
            ...
            log('[Nezha‑V0] TCP连接成功');

            // V0登录包
            const hostInfoRaw = await getHost();
            const loginPayload = {
                key: NEZHA_KEY,
                uuid: UUID,
                platform: hostInfoRaw.platform,
                platform_version: hostInfoRaw.platformVersion,
                cpu: hostInfoRaw.cpu,
                mem_total: hostInfoRaw.memTotal,
                disk_total: hostInfoRaw.diskTotal,
                swap_total: hostInfoRaw.swapTotal,
                arch: hostInfoRaw.arch,
                boot_time: hostInfoRaw.bootTime,
                version: AGENT_VERSION
            };
            function sendV0Packet(sock, obj) {
                const payload = Buffer.from(JSON.stringify(obj));
                const lenBuf = Buffer.alloc(4);
                lenBuf.writeUInt32BE(payload.length, 0);
                sock.write(Buffer.concat([lenBuf, payload]));
            }
            sendV0Packet(socket, loginPayload);
                ...
                socket.on('data', (chunk) => {
                    ...
                    while (buf.length >= 4) {
                        ...
                        try {
                            ...
                            // V0服务端下发任务，这里简单忽略任务，只做状态上报
                            if (msg.type === 'task') {
                                // v0任务处理可扩展，此处仅占位
                            }
                        } catch (e) {
                            logErr('[Nezha‑V0] parse packet fail', e.message);
                        }
                    }
                });
                socket.on('close', () => reject(new Error('socket close')));
                socket.on('error', (err) => reject(err));

                // 状态上报循环
                (async function stateReportLoop() {
                    try {
                        while (true) {
                            await trackNetworkSpeed();
                            const state = await getState();
            ...
            logErr('[Nezha‑V0] 连接异常，准备重连:', err.message);
        } finally {
            if (socket) {
                try { socket.destroy(); } catch (e) { }
            }
        }
        log(`[Nezha‑V0] ${RETRY_DELAY / 1000}s 后重试连接`);
        await sleep(RETRY_DELAY);
    }
}

// 入口分发：有NEZHA_PORT走V0，否则V1
async function startNezhaAgent() {
    if (!NEZHA_SERVER || !NEZHA_KEY) {
        console.log('[Nezha] NEZHA_SERVER 或 NEZHA_KEY 未配置，跳过哪吒 agent');
        return;
    }
    if (NEZHA_PORT && NEZHA_PORT.trim() !== '') {
        await startNezhaV0();
    } else {
        await startNezhaV1();
    }
...
httpServer.listen(PORT, () => {
    startNezhaAgent().catch(err => console.error('nezha agent error', err));
    addAccessTask();
